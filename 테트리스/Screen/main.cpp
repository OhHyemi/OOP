...
public:
	...
	int main()
	{
		...
		Block *block3 = new Block{ sprites3, 2,2, (5, 1) };
		Block *block4 = new Block{ sprites4, 4,4, (5, 1) };
		Block *block5 = new Block{ sprites5, 3,3, (5, 1) };
		...
		while (true)
		{
			...
			{
				random = next;
			    next = (unsigned)rand() % 6;
				
				nextBlock = blocks[next];
			}
			screen.drawRect((0,15), 10, 10);//다음블록알려주는판테두리
			screen.drawBlock(blocks[next]->getShape().c_str(), blocks[next]->getWidth(),blocks[next]->getHeight(), (0,20));//y이값이 안먹힘,. ㅠ
			//ui.render();
