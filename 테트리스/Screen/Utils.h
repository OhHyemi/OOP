...
class Screen {
	...
	Screen(int width = 30, int height = 30)
...
public:
	...
	void drawBlock(const char* shape, int w, int h, const Position& pos)
	{
		...
		for (int i = 0; i < h; i++)
		{
			...
			{
				if (shape[i * w + j] == '\xB0')
					strncpy(&canvas[pos.x + (pos.y + i )*(width + 1) + j], &shape[i * w + j], 1);
			}
		}
	}
...
public:
	Ui(int width = 50, int height = 30) :
		width(width), height(height), pos(pos), canvas(new char[(width)*height]) {}
